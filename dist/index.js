...
app.post('/user/sign', getImage, _index2.SignUser, _index2.LoginUser, _index3.GetFollows, _index.SignToken, _index.SignRefresh);
app.post('/login', _index2.LoginUser, _index3.GetFollows, _index.SignToken, _index.SignRefresh);
app.put('/user/edit', _index.VerifyToken, getImage, _index2.EditUser, _index2.GetUser);
app.delete('/user/unsign', _index2.LoginUser, _index2.DelUser);
app.post('/tweet/post', _index.VerifyToken, getImage, _index4.PostTweet, _index5.PostReply);
...
app.get('/profile', _index2.GetUser, _index4.GetTweets);
...
io.use(_index9.SocketAuthorization);
io.on('connection', socket => {
  socket.leave(socket.id); //초기 개인룸 삭제

  socket.join(socket.user_id); //고유 아이디로 재접속

  socket.on('login', () => (0, _index9.SocketLogin)(socket));
  socket.on('create room', (req, res) => (0, _index9.CreateRoom)(socket, req, res));
  socket.on('send message', (req, res) => (0, _index9.SendMessage)(socket, req, res));
  socket.on('leave room', (req, res) => (0, _index9.LeaveRoom)(socket, req, res));
});