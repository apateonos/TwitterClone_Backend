...
//timeline api
export const SELECT_TIMELINE = 'SELECT t.tweet_id, t.tweet_text, u.user_id, f.user_id, rt.user_id, rtf.user_id FROM tweets t LEFT JOIN  users u ON t.user_id = u.user_id LEFT JOIN follows f ON f.following_id = u.user_id LEFT JOIN retweets rt ON rt.tweet_id = t.tweet_id LEFT JOIN follows rtf ON rtf.following_id = rt.user_id WHERE u.user_id = 16 OR f.user_id = 16 OR rtf.user_id = 16;';
